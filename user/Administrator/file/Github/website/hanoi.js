/**
		...
		**/
		var Hnt = (function(){
			...
			Hnt.complete = false;
			Hnt.isPaused = false;
			Hnt.movie.pause = function() {
		Hnt.isPaused = true;
	};

// 继续播放
			Hnt.movie.resume = function() {
		if (Hnt.isPaused && order < movie.length) {
			Hnt.isPaused = false;
			play(); // Continue from where it was paused
		}
	};
			//日志对象赋值
			...
			//播放动画
	function play() {
		if (!Hnt.isStart || Hnt.isPaused) return;
